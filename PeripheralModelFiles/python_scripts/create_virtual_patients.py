...
vps = ['VP0', 'VP1', 'VP2', 'VP3', 'VP4']

for vp_no in vps:

    mus_and_sigmas = pd.concat([pd.read_csv('../text_files/u_and_sigmas_{}_{}.csv'.format(a, vp_no), sep=',', index_col=0, header=None)
                                for a in ['a12', 'a13', 'a14', 'a23']])
    print(mus_and_sigmas)

    virtual_patients = {}
    patient_fluxes = pd.read_csv('../SNL005_{}.txt'.format(vp_no), sep=' ', index_col=0)
    ...
    df.to_csv('../text_files/population_{}.csv'.format(vp_no))