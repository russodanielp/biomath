...
%%     Parameter Values:
     ...
     volume_en_P = 0.02;
     X15_degradation_rate_k = 0.0219;
     X18_degradation_rate_k = 0.1;
     X19_degradation_rate_k = 0.11;
     LDL_en_to_X10_rate_k = 7.55;
     X10_elimination_rate_k = 0;
     X10_production_rate_k = 72500;
     X21_production_rate_k = 7.62;
     SREBP_ic_to_nu_P_activation_rate_k = 0.0693;
     X22_degradation_rate_k = 0.693;
     X21_degradation_rate_k = 0.693;
     X16_production_rate_k = 0.6;
     LDLR_ic_to_cs_P_trafficking_rate_k = 0.2;
     X7_P_uptake_rate_k = 0;
     X7_P_LDLRuptake_rate_k = 0;
     HDL_to_VLDL_exchange_rate_k = 0.0238;
     X25_LDLRind_P_uptake_rate_k = 0.00034;
     ...
     Chol_er_exp = 10;
     X10_base = 1.7e+08;
     X1_to_pc_rate_k = 3.33;
     X27_to_pl_rate_k = 0.333;
     Statin_kmet = 1.25;
     StatinM_kel = 0.5;
     StatinM_Vd = 75;
     Fibrate_LPL_Emax = 1;
...
%      d(X(1))/dt = 1/compartment*(ReactionFlux6 - ReactionFlux7 - ReactionFlux57 - ReactionFlux58 + ReactionFlux59);
     ...
     S(7,45)    = -X(7)+LDLR_internalization_fraction_P*X(7)*X(12); %
     S(7,48)    = X(8)*chol_per_HDL_particle/chol_per_VLDL_particle*(weight_ApoB_HDL_VLDL*power(X(11),ApoB_HDL_VLDL_nh)/(power(ApoB_HDL_VLDL_EC50,ApoB_HDL_VLDL_nh)+power(X(11),ApoB_HDL_VLDL_nh)))*(weight_CETP_HDL_VLDL*power(CETP_pl,CETP_HDL_VLDL_nh)/(power(CETP_HDL_VLDL_EC50,CETP_HDL_VLDL_nh)+power(CETP_pl,CETP_HDL_VLDL_nh)));
     ...
     S(8,44)    = X(10)/chol_per_HDL_particle*power(X(9),ApoA1_RCT_nh)/(power(ApoA1_RCT_EC50,ApoA1_RCT_nh)+power(X(9),ApoA1_RCT_nh));
     ...
     S(10,35)   = -X(10);
     S(10,38)   = X(20)*chol_per_LDL_particle;
     S(10,43)   = -X(10)*power(X(9),ApoA1_RCT_nh)/(power(ApoA1_RCT_EC50,ApoA1_RCT_nh)+power(X(9),ApoA1_RCT_nh));
     S(10,46)   = (X7_P_uptake_rate_k*X(7)+X7_P_LDLRuptake_rate_k*LDLR_internalization_fraction_P*X(7)*X(12))*chol_per_VLDL_particle; %
     S(10,50)   = X(25)*chol_per_LDL_particle;
     S(11,27)   = X(11);
     S(12,9)    = X(16);
     S(12,13)   = -LDLR_internalization_fraction_P*X(12);
     S(12,14)   = X(15);
     S(12,15)   = -(X(25)/volume_pl)*(X(12)/volume_P)*volume_P;
     S(12,16)   = -(X(3)/volume_pl)*(X(12)/volume_P)*volume_P;
     S(12,19)   = LDL_LDLR_pl_Kd*X(13);
     S(12,20)   = PCSK9_LDLR_pl_Kd*X(17);
     S(13,15)   = (X(25)/volume_pl)*(X(12)/volume_P)*volume_P;
     S(13,19)   = -LDL_LDLR_pl_Kd*X(13);
     S(13,25)   = -LDLR_internalization_fraction_P*X(13);
     S(14,17)   = (X(20)/volume_en_P)*(X(15)/volume_en_P)*volume_en_P;
     S(14,21)   = -LDL_LDLR_en_Kd*X(14);
     S(14,25)   = LDLR_internalization_fraction_P*X(13);
     S(15,13)   = LDLR_internalization_fraction_P*X(12);
     S(15,14)   = -X(15);
     S(15,17)   = -(X(20)/volume_en_P)*(X(15)/volume_en_P)*volume_en_P;
     S(15,18)   = -(X(19)/volume_en_P)*(X(15)/volume_en_P)*volume_en_P;
     S(15,21)   = LDL_LDLR_en_Kd*X(14);
     S(15,22)   = PCSK9_LDLR_en_Kd*X(18);
     S(15,23)   = -X(15);
